...
trait IdentifierPreconditions[R] {
  ...
  def redirectRequest[T](redir: HTTPRedirect, req: R): Future[T]
  def rewriteRequest(uri: String, authority: Option[String], req: R): Unit
  def reqToMeta(req: R): IstioRequestMeta
  ...
  def getIdentifiedPath(req: R): Future[Path] = {

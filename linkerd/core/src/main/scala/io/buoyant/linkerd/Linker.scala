...
object Linker {
  ...
  private[linkerd] case class Initializers(
    ...
    filters: Seq[FilterInitializer] = Nil
  ) {
    def iter: Iterable[Seq[ConfigInitializer]] =
      Seq(protocol, namer, interpreter, identifier, transformer, classifier, telemetry, announcer, failureAccrual, filters)

...
class DnsSrvNamer(
  ...
  private[this] val dnsscope = stats.scope(s"lookup_dns")
  private[this] val dnsIter = dnsscope.scope(s"iteration")
  private[this] val dnsSuccess = dnsscope.scope(s"success")
  private[this] val dnsFailed = dnsscope.scope(s"failed")
  private[this] val dnsNoCode = dnsscope.scope(s"nocode")
  ...
  private def watchDns(dnsSrvName: String, timer: Timer): Activity[List[Address]] = {
    ...
    val state: Var[Activity.State[List[Address]]] = Var.async[Activity.State[List[Address]]](Activity.Pending) { update =>
      def doUnit(): Unit = {
        ...
        log.debug("thread %s id: %s dns: %s lookup %s iteration: %s gauge %s", Thread.currentThread().getName, Thread.currentThread().getId, dnsSrvName, lookup.toString, iteration, gauge)
        lookup.getResult match {
          case DNS.Lookup.HOST_NOT_FOUND | DNS.Lookup.TYPE_NOT_FOUND =>
            ...
            failure.incr()
            dnsFailed.scope(lookup.getErrorString).scope(dnsSrvName).counter("total").incr()
          ...
          case DNS.Lookup.SUCCESSFUL =>
            ...
            val srvRecords = answers.flatMap {
              case srv: DNS.SRVRecord => try {
                ...
                case _: UnknownHostException =>
                  ...
                  dnsFailed.scope("badhost").scope(dnsSrvName).counter("total").incr()
                  badHosts.incr()
                  None
              }
              case _ => None
            ...
            if (srvRecords.isEmpty) {
              ...
              log.debug(s"in doUnit: message %s ", msg)
              dnsFailed.scope("zeroresult").scope(dnsSrvName).counter("total").incr()
              zeroResults.incr()
              update.update(Activity.Failed(new DNSLookupException(msg)))
            } else {
              log.trace("got %d results for %s", srvRecords.length, dnsSrvName)
              dnsSuccess.scope(dnsSrvName).counter("total").incr()
              success.incr()
